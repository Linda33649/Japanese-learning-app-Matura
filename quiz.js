...
document.addEventListener("DOMContentLoaded", () => {
    ...
    answerButtons.forEach(buttonId => { //adds a click event listener to each answer button and checks if a button has already been clicked e.g. the question has been aswered or not using the boolean Answered
    ...
    document.addEventListener("click", function () { //reloads the page when the page is clicked after the question has been answered
        if (Answered) {
            location.reload()
            Answered = false
        }
    ...
    function checkAnswer(event) { //checks if the clicked button is the same as the one that was selected to display the correct answer on and then adds the corresponding class to color the button via css
    ...
    if (QuestionCount === QuizLength) { //Ends the quiz using the variable that has the amount of answered questions stored and the variable that sets the length of the quiz depending on how many characters a quiz has
